...
impl Element {
  ...
  pub fn attributes(&self) -> JsResult<JsValue> {
    self
      .0
      .get()
      .map(|e| e.attributes().iter().map(Attribute::from).collect::<Vec<_>>())
  ...
  pub fn set_attribute(&mut self, name: &str, value: &str) -> JsResult<()> {
    self.0.get_mut()?.set_attribute(name, value).into_js_result()
  }
  ...
  pub fn prepend(&mut self, content: &str, content_type: Option<ContentTypeOptions>) -> Result<(), JsValue> {
    self.0.get_mut().map(|e| e.prepend(content, content_type.into_native()))
  }
  ...
  pub fn set_inner_content(&mut self, content: &str, content_type: Option<ContentTypeOptions>) -> Result<(), JsValue> {
    self
      .0
      .get_mut()
