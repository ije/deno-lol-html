...
use lol_html::{AsciiCompatibleEncoding, HtmlRewriter as NativeHTMLRewriter, OutputSink, Selector, Settings};
use std::borrow::Cow;
...
impl HTMLRewriter {
  ...
  fn inner_mut(&mut self) -> JsResult<&mut NativeHTMLRewriter<'static, JsOutputSink>> {
    match self.0 {
      RewriterState::Before { .. } => {
        if let RewriterState::Before { settings, output_sink } = std::mem::replace(&mut self.0, RewriterState::After) {
  ...
  pub fn on(&mut self, selector: &str, handlers: ElementContentHandlers) -> JsResult<()> {
    match self.0 {
      RewriterState::Before { ref mut settings, .. } => {
  ...
  pub fn on_document(&mut self, handlers: DocumentContentHandlers) -> JsResult<()> {
    match self.0 {
      RewriterState::Before { ref mut settings, .. } => {
        settings.document_content_handlers.push(handlers.into_native());
        Ok(())
      }
      _ => Err(JsError::new("Handlers cannot be added after write").into()),
    }
